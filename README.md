...
331GB의 데이터를 모두 사용하기에는 시간이 많이 소요되므로 데이터 수를 줄이고, Epoch 수를 늘리는 방안을 택한다. 데이터셋은 train_set 80%, test_set 20%로 사용하며, 결과에 따라 유동적으로 변경할 수 있다.

### 2_3. 쓰레기 분류 방식
판별된 결과에 따라 쓰레기를 이동시켜 분류한다. 이를 위한 두 가지 방법이 있다.
...
### 1) 컨베이어 시스템
  - 1_1) 초음파 센서로 물체 감지
  - 1_2) 물체가 감지되면 카메라 촬영 위치까지 컨베이어 이동
  - 1_3) 물체 판별 후 분류를 위한 컨베이어 이동

### 2) 쓰레기 종류 파악 시스템
  - 2_1) 컨베이어에서 카메라 촬영 위치까지 물체 이동
  - 2_2) 촬영 후 이미지 크기 640*640으로 조정
  - 2_3) 학습된 모델로 쓰레기 종류 판별
